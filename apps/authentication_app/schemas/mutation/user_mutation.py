...
class IsVerified(graphene.Mutation):
    ...
    def mutate(cls, root, info, **kwargs):
        ...
        if not user:
            raise GraphQLError(f"user with {email} does not exist, Please SignUp ")
        elif user.is_verified:
            raise GraphQLError(f"user with {email} is aleady verified")
        elif not user.OTP == input_otp:
            print(f'>>>>>>>>>>>>>>>>>>>>>>>>>>>>> {input_otp} {user.OTP}') 
            raise GraphQLError("User can not be verified, wrong otp!")
        elif diff > 60000:
            raise GraphQLError(" OTP has been expired, Please send another refresh OTP ")
        else:
...
class RequestChangPassword(graphene.Mutation):
    class Arguments:
        email = graphene.String(required=True)
    message = graphene.String()
